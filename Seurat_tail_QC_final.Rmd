...
tail <- subset(tail, subset = nFeature_RNA > 200 & percent.mt < 25) # nFeature_RNA > 200 is standard. Mitochondrial DNA cutoff is commonly 25% and I confirmed it based on these plots.
```
...
Normalize and Scale, regressing by Sample and nFeature_RNA to get rid of projection separation of these aspects
```{r}
...
Load previously identified monocle cell types- this was useful when adding initial annotations, not necessary to rerun if clusters are already identified
```{r}
...
```{r}
# You can see this by looking at nFeature_RNA, and it's supported by the percent mt and rb
...
# Run before integration to confirm how this change affects grouping
...
Integrate it again so that the Samples don't batch separately as much!
```{r}
# Integrate by Sample
tail_int <- tail_scale
tail_int[["RNA"]] <- split(tail_int[["RNA"]], f = tail_int$Sample)
